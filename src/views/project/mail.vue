...
<script>
import { getMailTemplatesList, delMailTemplates } from '@/api/table'
...
export default {
  ...
  components: {
    formDialog: () => import('./mailForm.vue')
  },
  data() {
    return {
      list: null,
  ...
  methods: {
    fetchData(tag) {
      ...
      getMailTemplatesList(this.listQuery).then(response => {
    ...
    showInfo(id) {
      ...
      this.$router.push({path: '/project/mail/' + id})
    },
    appHandle () {
